"""Network helper class for the network integration (IPv4 only, IPv6 commented out)."""
...
from .models import Adapter, IPv4ConfiguredAddress

_LOGGER = logging.getLogger(__name__)


async def async_load_adapters() -> list[Adapter]:
    """Load adapters (IPv4 only)."""
...
def _ip_address_is_external(ip_addr: IPv4Address) -> bool:
    return (
...
def _ifaddr_adapter_to_ha(
    adapter: ifaddr.Adapter, next_hop_address: IPv4Address | None
) -> Adapter:
    """Convert an ifaddr adapter to ha (IPv4 only)."""
...
def async_get_source_ip(target_ip: str) -> str | None:
    """Return the source IPv4 that will reach target_ip."""
    ...
    try:
        ...
        _LOGGER.debug(
            (
                "The system could not auto detect the IPv4 source ip for %s"
            ),
            target_ip,
        )
        return None
